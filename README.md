...
```
ibmcloud ks cluster-config --cluster YOUR-CLUSTER-NAME-HERE
```

3. Set the KUBECONFIG environment variable. Copy the output from the previous command and paste it in your terminal. The command output looks similar to the following example:

See your kubernetes cluster's *ACCESS* tab for the exact command

```
export KUBECONFIG=/Users/$USER/.bluemix/plugins/container-service/clusters/PATH/TO/CLUSTER/YAML
```
...
```bash
helm upgrade --install cicd . --atomic --values=config.yaml --namespace cicd
```
